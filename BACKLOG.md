...
| Code/functional test coverage measurement | No coverage tooling is wired in (e.g. `@vitest/coverage-v8`) -- the 4-tier testing strategy is applied by judgment story-by-story, but nothing measures or reports how much of the codebase it actually exercises, or fails CI below a threshold | 2026-09-09 |
| Dependency/package security scanning | No automated vulnerability scanning on dependencies (`npm audit` as a CI gate, Dependabot, Snyk, etc.). `npm ci` already surfaces vulnerability counts inline in CI output, but nothing currently reads or acts on them | 2026-09-09 |
| OWASP-style security testing | No structured security review (OWASP Top 10 checklist, or a scanner like OWASP ZAP) has been run against the app. Worth scoping once real minors' data is actually live in a hosted environment, not just local dev -- see "Host a public demo on Vercel" row above | 2026-09-09 |
