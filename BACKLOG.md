...
| Code/functional test coverage measurement | No coverage tooling is wired in. Plan: `@vitest/coverage-v8` producing a report each CI run (visibility first), then a **patch-coverage** gate -- new/changed lines in a PR must be covered, rather than a blunt whole-repo threshold that punishes untouched legacy code | 2026-09-09 |
| Dependency/package security scanning | No automated dependency scanning. Plan: **Dependabot** (`.github/dependabot.yml`) for both security-CVE PRs and grouped version-bump PRs, plus `npm audit` visible in CI. `npm ci` already prints vulnerability counts that nothing acts on | 2026-09-09 |
| OWASP-style security testing | No structured security review has been run. Plan: start with **CodeQL** (GitHub SAST -- injection, path traversal, SSRF, ReDoS, missing-auth patterns; free for public repos) and **secret scanning + push protection** (one toggle), both as CI checks. DAST (OWASP ZAP) / a real pentest are the heavier follow-on, worth it once real minors' data is live in a hosted environment -- see "Host a public demo on Vercel" above | 2026-09-09 |
| `next build` not run in CI | CI runs `tsc --noEmit` + tests but never `next build`, so a change that type-checks and passes tests but breaks the actual production bundle (bad `use client`/server boundary, a build-time-only error, an env var only read at build) wouldn't be caught until deploy. Cheap to add as a CI step | 2026-09-10 |
| No pre-push / pre-commit git hooks | `npm run lint` / `npm test` are only enforced in CI, so the first place a mistake surfaces is a red PR check ~1 min later. Plan: Husky + `lint-staged` (pre-commit: eslint --fix + prettier on staged files only, sub-second) and a pre-push hook (`npm run lint` + `npm test`, or `vitest --changed` as the suite grows). Bypassable by design -- fast feedback, not a security boundary; CI stays the real gate | 2026-09-10 |
| Accessibility testing | No accessibility audit (WCAG conformance, screen reader pass, automated tooling like axe/Lighthouse) has been done on any page. Current UI is plain semantic HTML with real `<label>`/`role="alert"` usage, but none of that has been verified against an actual standard | 2026-09-09 |
| AI usage efficiency | No visibility into or established practice for measuring how efficiently Claude Code's tool/token usage is translating into shipped value on this project. Worth a deliberate look at whether the current workflow's overhead (a full branch+PR+CI cycle per change however small, a full lint/test/migrations/e2e run on every restack in a stack, live browser verification and throwaway smoke-test PRs for confidence) is well-calibrated to this project's actual scale, or where it's worth trimming as the backlog grows | 2026-09-11 |
